...
const customFile = path.join(skillsRoot, 'codex-scientific-reading', 'SKILL.md');
...
async function ps(args, label) {
  ...
  const code = await new Promise((resolve, reject) => {
    const child = spawn(path.join(process.env.SYSTEMROOT, 'System32', 'WindowsPowerShell', 'v1.0', 'powershell.exe'),
      ['-NoProfile', '-ExecutionPolicy', 'Bypass', ...args], { windowsHide: true, shell: false, stdio: ['ignore', out.fd, err.fd] });
...
try {
  ...
  const installArgs = ['-File', path.join(candidate, 'install.ps1'), '-Root', root,
    '-PluginArchive', path.join(candidate, 'inputs', 'scientific-reading.tgz'), '-InstallSkill', '-SkillsDirectory', skillsRoot];
  const result = await ps(installArgs, 'install');
  ...
  check('actual PowerShell 5.1 installer selects the new good build', selected.appSha256 !== original.appSha256);
  ...
  await ps(['-File', path.join(root, 'workbench.ps1'), 'rollback'], 'rollback');
  ...
  check('rollback preserves current artifact bytes and library identities', JSON.stringify(await artifacts(path.join(root, 'library'))) === JSON.stringify(before)
    && JSON.stringify(await paperIds()) === JSON.stringify(papers));
  await ps(installArgs, 'reinstall');
