...
try {
  ...
  const binding1 = await invoke('bind', { paperId: paper1.paper_id });
  const binding2 = await invoke('bind', { paperId: paper2.paper_id });
  check('two papers have different native sessions', binding1.sessionId !== binding2.sessionId);
  check('native session reuses stable binding', (await invoke('bind', { paperId: paper1.paper_id })).sessionId === binding1.sessionId);
  ...
  check('rename preserves session', (await invoke('bind', { paperId: paper1.paper_id })).sessionId === binding1.sessionId);
  ...
  await invoke('bind', { paperId: paper1.paper_id });
  ...
  check('genuine native child inherits paper query and write boundary', probe.childListing.isError === false
  ...
  check('genuine native child reads the parent paper context', probe.childContext.isError === false
    && JSON.stringify(probe.childContext).includes(paper1.paper_id) && !JSON.stringify(probe.childContext).includes(paper2.paper_id));
  ...
  check('moved paper continues the original chat and task', moved.status !== 'failed' && moved.sessionId === binding1.sessionId && moved.jobId === task1.jobId && moved.folderId === folder2.folder_id);
  await invoke('folder_archive', { folderId: folder2.folder_id, archived: true });
  const archived = await invoke('task', { taskId: task2.taskId });
  check('archived category invalidates old task', archived.status === 'failed' && ['folder_archived', 'scope_changed'].includes(archived.error));
