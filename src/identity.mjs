...
export function apply(ctx) {
  const identity = { ...JSON.parse(process.env.CSR_IDENTITY), pid: process.pid };
  ...
  const disposePage = ctx.webServer.register({ kind: 'exact', path: '/__workbench',
    handler(request, response) {
      ...
      response.end(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
        <title>Codex 文献工作台 · 实例</title><style>body{font:16px/1.6 system-ui;margin:4rem auto;padding:0 1.5rem;max-width:46rem;color:#24303a}dt{color:#64707a;margin-top:1rem}dd{margin:0;overflow-wrap:anywhere}a{display:inline-block;margin-top:2rem;color:#245ecc}</style>
        <h1>文献工作台已启动</h1><p>${identity.candidate ? '发布候选版本' : '版本'} ${escape(identity.version)}</p>
        <dl><dt>产品</dt><dd>${escape(identity.product)}</dd><dt>实例 ID</dt><dd>${escape(identity.instanceId)}</dd>
        <dt>本次启动 ID</dt><dd>${escape(identity.launchId)}</dd></dl><p><a href="/">进入 DSH 文献工作台</a></p>
