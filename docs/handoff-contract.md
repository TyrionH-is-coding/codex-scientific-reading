# v0.2 单篇 chat 与机器交接合同

当前候选版每篇论文以 SQLite paperId → sessionId 绑定原生 chat。`bind` 接收 paperId；`submit` 接收 paperId 与稳定 idempotencyKey，folderId 可选。移动分类和重启后继续原会话、原任务。

B handoff 格式 1 先备份为 `state/handoff.v1.json` 再迁移到格式 2；原分类绑定登记为只读历史，旧 Review 保留原论文关系。A 库格式升级到 6，跨格式不能原地向后回退。

宿主按实际根会话及子会话谱系解析单篇范围，Python 和 worker 在读取/写入/发布时复核。归档论文暂停受限操作，管理员可恢复分类。旧分类任务的历史边界不放宽；用户新开本篇 chat 后才用新的单篇合同。

多选历史、Figure、研究字段、Reader 与雷达的机器合同见 [v0.2 API](../skills/deep-literature-for-codex/references/v02.md)。真实模型回答质量、真实 OAuth 和机构权限分别验收，不由合成工具闭环推断。

以下记录对应旧版本，不用于创建 v0.2 分类主管；原生 RPC 的形状仍可参考。
