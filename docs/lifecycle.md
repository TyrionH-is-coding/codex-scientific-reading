...
rc.7 的 Excel 管理增加了格式 5。安装器允许格式 4 → 5 升级：先停止实例并保存完整库快照，再启用新程序；A 引擎首次访问旧库时备份 SQLite 并迁移。已选择新版后若启动失败，保留恢复标记，通过 `recover` 继续启动新版，不自动启动格式 4 的旧程序。备份失败时不切换版本。格式 5 → 4 回退会被拒绝；确需查看升级前数据，请把旧快照恢复到另一个独立实例，勿覆盖现有库。
