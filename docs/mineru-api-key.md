...
本工作台使用需要 Token 的精准解析 API，Token 可免费获取。官网另外提供的免 Token 轻量接口，不能直接替代这里的配置。

### 免费额度大约能读多少篇？

截至 2026-09-08，MinerU 当前中文 [API 文档](https://mineru.net/apiManage/docs)说明：

| 项目 | 当前限制 |
| --- | --- |
| 每日最高优先级解析额度 | 每个账号 **1,000 页**；超出后降低解析优先级 |
| 单个 PDF 页数 | 最多 **200 页** |
| 单个文件大小 | 最多 **200 MB** |

按每篇 10–20 页估算，每天约相当于 **50–100 篇论文**的高优先级解析。实际额度与排队情况以自己的账号页面为准。免费 Token 获取方式见 [MinerU 官方生态说明](https://github.com/opendatalab/MinerU-Ecosystem/blob/main/README.zh-CN.md)。
...
**保存和“重新检测”用于检查配置状态，不会代你发起真实解析。** 要确认 Key 与服务都可用，继续[使用指南中的第一篇论文流程](getting-started.md#第三步获取并精读第一篇论文)：
