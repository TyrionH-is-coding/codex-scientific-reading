...
| **长期管理 · Excel + 本地文献库** | 按分类、标签、个人阅读进度和笔记查找论文 | 六项个人记录回写；阅读成果和图表可回查原文 |
...
当前使用固定 Reader 模板和 Excel 字段；自定义模板、自定义字段、文献雷达尚未提供。新表已完成 Windows Excel 原生打开、中文编辑、整表排序、保存与回写；macOS Excel / Numbers 和 Linux LibreOffice 的桌面回写仍需实机核验。

Excel 的 [v0.1 设计与历史审查](docs/excel-v0.1-redesign.md) 及 [当前验收记录](docs/excel-v0.1-acceptance.md) 分别保存设计依据和实际结果，发布状态以 Release 为准。

[v0.2 技术路线](docs/roadmap-v0.2.md) 已记录下一版方向：每篇文献对应一个持久 DSH chat，用户可多选 chat 交由 Codex 做跨文献总结，并继续规划 Excel 字段、HTML 模板自定义和文献雷达。当前 v0.1 仍采用分类主管模式。
