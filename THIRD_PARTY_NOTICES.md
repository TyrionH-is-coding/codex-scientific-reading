...
| DeepSeek Harness | npm 官方 `@deepseek-ai/dsh` 0.1.5-rc.1；宿主与 peer 依赖见 lock | 各 npm 包的 `LICENSE` 和 package 元数据 |
...
| tinycss2 / webencodings | 官方 PyPI wheel 1.5.1 / 0.6.1，版本与 SHA 见 Python lock，用于 Reader CSS 解析 | 各 wheel 的 dist-info/licenses/LICENSE，BSD-3-Clause |
...
未随包分发完整第三方研究 Skill；文献雷达由本项目共享引擎实现，通过公开元数据接口发现候选。安装器下载的解释器和依赖保留原始许可文件；研究 PDF、用户库、凭据和本机测试记录不进入发行源码归档。
